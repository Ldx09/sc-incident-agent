...
def save_audit(
    ...
    """
    Save the full audit trail for an incident.
    actions is a list of:
      {priority, action, owner, deadline, was_taken, outcome, notes}
    """
    init_audit()
    conn = get_connection()

    conn.execute(
        "DELETE FROM action_audits WHERE incident_id = ?",
        (incident_id,)
    )
    conn.execute(
        "DELETE FROM incident_outcomes WHERE incident_id = ?",
        (incident_id,)
    )
    ...
    for a in actions:
        conn.execute("""
            INSERT INTO action_audits (
                incident_id, action_priority, action_text,
                action_owner, action_deadline,
                was_taken, outcome, notes, recorded_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            incident_id,
            a["priority"],
...
def get_audit(incident_id: str) -> dict | None:
    """Return full audit data for one incident."""
    init_audit()
    conn = get_connection()

    actions = conn.execute(
        "SELECT * FROM action_audits WHERE incident_id = ? ORDER BY action_priority",
        (incident_id,)
    ).fetchall()

    outcome = conn.execute(
        "SELECT * FROM incident_outcomes WHERE incident_id = ?",
        (incident_id,)
    ).fetchone()
...
def get_audit_stats() -> dict:
    ...
    taken = conn.execute(
        "SELECT COUNT(*) FROM action_audits WHERE was_taken = 1"
    ).fetchone()[0]
    ...
    overall_rows = conn.execute("""
        SELECT overall_outcome, COUNT(*) as count
    ...
    incidents_audited = conn.execute(
        "SELECT COUNT(DISTINCT incident_id) FROM incident_outcomes"
    ).fetchone()[0]
