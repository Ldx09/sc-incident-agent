...
def generate_playbook_pdf(playbook: dict) -> bytes:
    ...
    if playbook.get("incident_type_description"):
        section("Overview"); body(playbook["incident_type_description"])
    if playbook.get("early_warnings"):
        section("Early warning signals"); bullet_list(playbook["early_warnings"])
    ...
    if playbook.get("root_causes"):
        section("Common root causes"); bullet_list(playbook["root_causes"])
    if playbook.get("what_worked"):
        section("What worked"); bullet_list(playbook["what_worked"])
    if playbook.get("what_failed"):
        section("What failed or made things worse"); bullet_list(playbook["what_failed"])
    ...
    if playbook.get("key_contacts"):
        ...
        for c in playbook["key_contacts"]:
            if isinstance(c,dict): body(f"{c.get('role','')}: {c.get('why_needed','')}")
            else: body(str(c))
    ...
    if playbook.get("lessons"):
        section("Key lesson — never forget this"); body(playbook["lessons"])
...
def render_playbook_card(playbook: dict):
    ...
    conf_color = {"high":"#1D9E75","medium":"#BA7517","low":"#D85A30"}.get(conf,"#888")
    ...
    st.markdown(
        ...
        f'background:var(--color-background-secondary);color:var(--color-text-secondary)">{label}</span>'
        f'<span style="font-size:11px;color:{conf_color};font-weight:500">● {conf.upper()} confidence</span>'
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "Early warnings","First 24 hours","Root causes & lessons",
        "What worked / failed","Contacts & metrics"])

    with tab1:
        st.markdown("**Signals to watch for — before it becomes critical:**")
        for w in playbook.get("early_warnings",[]):
            st.markdown(
                ...
                f'<span style="font-size:13px">{w}</span></div>', unsafe_allow_html=True)

    with tab2:
        st.markdown("**Priority actions — first 24 hours:**")
        for i, action in enumerate(playbook.get("first_24h",[]),1):
            ...
            st.markdown(
                ...
                f'border-bottom:0.5px solid var(--color-border-tertiary)">'
                f'<span style="font-weight:600;color:#D85A30;min-width:22px;font-size:13px">{i}</span>'
                f'<div><div style="font-size:11px;color:var(--color-text-tertiary);'
                f'font-weight:500;text-transform:uppercase;letter-spacing:.04em">{owner}</div>'
                f'<div style="font-size:13px;color:var(--color-text-primary)">{task}</div>'
    ...
    with tab3:
        ...
        with col_rc:
            ...
            for rc in playbook.get("root_causes",[]): st.markdown(f"~ {rc}")
        with col_lesson:
            ...
            if playbook.get("lessons"):
                st.markdown(
                    ...
                    f'line-height:1.6;color:#26215C">{playbook["lessons"]}</div>',
                    unsafe_allow_html=True)
        if playbook.get("prevention_recommendations"):
            st.markdown("**Prevention recommendations:**")
            for p in playbook["prevention_recommendations"]: st.markdown(f"→ {p}")
            ...
            if worked:
                for w in worked:
                    st.markdown(
                        ...
                        unsafe_allow_html=True)
            else: st.caption("No audit data yet — run audits to populate this.")
        with col_f:
            ...
            if failed:
                for f in failed:
                    st.markdown(
                        ...
                        unsafe_allow_html=True)
            else: st.caption("No failure data recorded yet.")
                ...
                for c in contacts:
                    if isinstance(c,dict):
                        st.markdown(
                            ...
                            f'<span style="font-size:13px;font-weight:500">{c.get("role","")}</span>'
                            f'<br><span style="font-size:12px;color:var(--color-text-secondary)">'
                            f'{c.get("why_needed","")}</span></div>', unsafe_allow_html=True)
                    else: st.markdown(f"• {c}")
            else: st.caption("No contact data yet.")
        with col_m:
            ...
            if metrics:
                for m in metrics:
                    st.markdown(
                        ...
                        unsafe_allow_html=True)
            else: st.caption("No metrics defined yet.")
...
def main():
    ...
    type_counts    = get_incident_type_counts(user_id=user_id)
    existing       = get_all_playbooks(user_id=user_id)
    existing_types = {p["incident_type"] for p in existing}

    if not type_counts:
        st.info("No incidents analyzed yet. Go to the main page and run some analyses.")
    ...
    for col, (inc_type, count) in zip(gen_cols, type_counts.items()):
        ...
        with col:
            st.markdown(
                ...
                f'var(--color-background-secondary);border-radius:8px;margin-bottom:6px">'
                f'<div style="font-size:13px;font-weight:500">{label}{badge}</div>'
                f'<div style="font-size:11px;color:var(--color-text-tertiary)">{count} incident(s)</div>'
                f'</div>', unsafe_allow_html=True)
            btn_label = "Re-generate" if has_one else "Generate"
            if st.button(btn_label, key=f"gen_{inc_type}", use_container_width=True):
                with st.spinner(f"Synthesizing {count} incident(s) into a {label} playbook..."):
                    try:
                        playbook = generate_playbook(inc_type, user_id=user_id)
                        if playbook:
                            st.success(f"Playbook generated from {count} incident(s).")
                            st.rerun()
                        else:
    ...
    for playbook in existing:
        ...
        label    = TYPE_LABELS.get(inc_type, inc_type.replace("_"," ").title())
        with st.expander(
            f"**{playbook.get('title',label)}**  |  {label}  |  "
            ...
            with col_pdf:
                try:
                    ...
                    st.download_button(
                        ...
                        file_name=f"{inc_type}_playbook.pdf",
            ...
            with col_regen:
                if st.button("Re-generate", key=f"regen_{inc_type}", use_container_width=True):
                    with st.spinner("Re-synthesizing..."):
                        try:
                            generate_playbook(inc_type, user_id=user_id)
            ...
            with col_del:
                if st.button("Delete", key=f"del_{inc_type}", type="secondary", use_container_width=True):
                    delete_playbook(inc_type, user_id=user_id)
