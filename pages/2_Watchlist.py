...
def risk_gauge(score: int, key: str):
...
<script>
new Chart(document.getElementById('g{key}'),{{
  type:'doughnut',
...
def hits_chart(hits: list[dict], key: str):
    if not hits:
        return
    recent = hits[:10]
    labels = [h["incident_id"].split("-")[-1] for h in reversed(recent)]
    colors = [SEV_COLORS.get(h["severity"],"#888") for h in reversed(recent)]
    values = [{"CRITICAL":4,"HIGH":3,"MEDIUM":2,"LOW":1}.get(h["severity"],1) for h in reversed(recent)]
    html = f"""<!DOCTYPE html><html><head>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"></script>
</head><body style="margin:0;background:transparent">
<div style="position:relative;width:100%;height:160px">
<canvas id="h{key}"></canvas></div>
<script>
new Chart(document.getElementById('h{key}'),{{
  type:'bar',
  data:{{labels:{json.dumps(labels)},datasets:[{{label:'Severity',data:{json.dumps(values)},
    backgroundColor:{json.dumps(colors)},borderRadius:4,borderSkipped:false}}]}},
  options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},
    scales:{{y:{{min:0,max:4,ticks:{{stepSize:1,
      callback:function(v){{return ['','LOW','MEDIUM','HIGH','CRITICAL'][v]||''}}}},
      grid:{{color:'rgba(0,0,0,0.06)'}}}},x:{{ticks:{{font:{{size:10}}}}}}}}}}
}});
</script></body></html>"""
...
def main():
    ...
    with st.expander("Add supplier to watchlist", expanded=len(watchlist) == 0):
        ...
        with col_a:
            new_supplier = st.text_input("Supplier name",
                placeholder="e.g. TechComp Taiwan", label_visibility="collapsed")
        with col_b:
        ...
        with col_c:
            if st.button("Add", type="primary", use_container_width=True):
                if new_supplier.strip():
                    added = add_supplier(new_supplier.strip(), new_notes.strip(), user_id=user_id)
                    if added:
                        st.success(f"Added **{new_supplier}** to watchlist.")
                        st.rerun()
                    else:
                        st.warning(f"**{new_supplier}** is already on your watchlist.")
                else:
    ...
    sort_by = st.radio("Sort by",
        ["Risk score","Incident count","Last seen","Name"], horizontal=True, index=0)
    sort_map = {
    ...
    for i, entry in enumerate(sorted_list):
        ...
        hits   = get_hits_for_supplier(entry["supplier"], user_id=user_id)
        last   = (entry["last_seen"] or "")[:16].replace("T", " ")
        key    = f"{entry['supplier']}_{i}".replace(" ","_")

        with st.expander(
            f"**{entry['supplier']}** — {label}  |  "
            f"score: {score}/10  |  incidents: {entry['incident_count']}",
            expanded=(score >= 7)
        ):
            col_gauge, col_info, col_hist = st.columns([1, 2, 2])

            with col_gauge:
                risk_gauge(score, key)
                st.markdown(
            ...
            with col_info:
                ...
                if last: st.markdown(f"**Last seen in incident:** {last}")
                st.markdown(f"**Total incidents:** {entry['incident_count']}")
                current_notes = entry.get("notes") or ""
                new_note = st.text_area("Notes", value=current_notes, height=80,
                    key=f"notes_{key}",
                    placeholder="Sole source? Annual spend? Contract expiry?")
                col_save, col_del = st.columns(2)
                with col_save:
                    if st.button("Save notes", key=f"save_{key}", use_container_width=True):
                        update_notes(entry["supplier"], new_note, user_id=user_id)
                        st.success("Saved.")
                with col_del:
                    if st.button("Remove", key=f"del_{key}", type="secondary", use_container_width=True):
                        remove_supplier(entry["supplier"], user_id=user_id)
                ...
                if hits:
                    ...
                    for h in hits[:5]:
                        ...
                        st.markdown(
                            ...
                            f'border-bottom:1px solid rgba(0,0,0,0.06);font-size:12px">'
                            f'<span style="font-family:monospace">{h["incident_id"]}</span>'
                            f'<span style="color:{sev_color};font-weight:500">{h["severity"]}</span>'
                            f'<span style="color:#888">{(h["hit_at"] or "")[:10]}</span>'
                    ...
                    st.caption(
                        "This supplier hasn't appeared in any incident analysis yet. "
                        "Risk score will update automatically when they do.")
