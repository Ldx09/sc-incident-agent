...
VULN_COLORS = ["#7F77DD","#1D9E75","#D85A30","#378ADD","#BA7517","#A32D2D","#639922","#D4537E"]
TYPE_COLORS = ["#378ADD","#7F77DD","#1D9E75","#D85A30","#BA7517","#639922","#D4537E","#A32D2D"]
...
new Chart(document.getElementById('c'),{{
  ...
  data:{{labels:{json.dumps(labels)},datasets:[{{data:{json.dumps(values)},
    backgroundColor:{json.dumps(colors)},borderWidth:2,borderColor:'#ffffff'}}]}},
...
new Chart(document.getElementById('c'),{{
  type:'bar',
  data:{{labels:{json.dumps(clean)},datasets:[{{label:'Incidents',data:{json.dumps(values)},
    backgroundColor:{json.dumps(colors)},borderRadius:4,borderSkipped:false}}]}},
  options:{{indexAxis:'y',responsive:true,maintainAspectRatio:false,
    plugins:{{legend:{{display:false}}}},
    scales:{{x:{{ticks:{{stepSize:1}},grid:{{color:'rgba(0,0,0,0.06)'}}}},
...
new Chart(document.getElementById('c'),{{
  type:'bar',
  data:{{labels:{json.dumps(clean)},datasets:[{{label:'Count',data:{json.dumps(values)},
    backgroundColor:{json.dumps(colors)},borderRadius:4,borderSkipped:false}}]}},
  options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},
    scales:{{y:{{ticks:{{stepSize:1}},grid:{{color:'rgba(0,0,0,0.06)'}}}},
...
def risk_timeline(incidents: list):
    ...
    labels = [r["incident_id"].split("-")[-1]+" "+r["severity_level"] for r in rows]
    values = [r["risk_score_overall"] for r in rows]
    html = f"""<!DOCTYPE html><html><head>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"></script>
</head><body style="margin:0;background:transparent">
<div style="position:relative;width:100%;height:240px"><canvas id="c"></canvas></div>
<script>
new Chart(document.getElementById('c'),{{
  type:'line',
  data:{{labels:{json.dumps(labels)},datasets:[{{label:'Risk score',data:{json.dumps(values)},
    borderColor:'#D85A30',backgroundColor:'rgba(216,90,48,0.08)',borderWidth:2,
    pointBackgroundColor:'#D85A30',pointRadius:4,tension:0.3,fill:true}}]}},
  options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},
    scales:{{y:{{min:0,max:10,ticks:{{stepSize:2}},grid:{{color:'rgba(0,0,0,0.06)'}}}},
...
def financial_bar(incidents: list):
    ...
    mins   = [(r["financial_min"] or 0)//1000 for r in rows]
    maxs   = [((r["financial_max"] or 0)-(r["financial_min"] or 0))//1000 for r in rows]
    html = f"""<!DOCTYPE html><html><head>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"></script>
</head><body style="margin:0;background:transparent">
<div style="position:relative;width:100%;height:260px"><canvas id="c"></canvas></div>
<script>
new Chart(document.getElementById('c'),{{
  type:'bar',
  data:{{labels:{json.dumps(labels)},datasets:[
    {{label:'Min ($K)',data:{json.dumps(mins)},backgroundColor:'rgba(55,138,221,0.5)',borderRadius:3,borderSkipped:false}},
    {{label:'Additional exposure ($K)',data:{json.dumps(maxs)},backgroundColor:'rgba(216,90,48,0.6)',borderRadius:3,borderSkipped:false}}
  ]}},
  options:{{responsive:true,maintainAspectRatio:false,
    plugins:{{legend:{{position:'top',labels:{{font:{{size:11}}}}}}}},
    scales:{{x:{{stacked:true,ticks:{{font:{{size:11}}}}}},
...
def audit_section(stats: dict):
    ...
    c4.metric("Follow-through rate", f"{ft}%")
    ...
    with col_outcome:
        ...
        if oc:
            ...
            label_map = {
                ...
                "escalated":"Escalated","ongoing":"Ongoing",
                "skipped":"Skipped","not_recorded":"Not recorded",
            }
            color_map = {
...
<div style="position:relative;width:100%;height:220px"><canvas id="oc"></canvas></div>
<script>
new Chart(document.getElementById('oc'),{{
  type:'doughnut',
  data:{{labels:{json.dumps(clean_labels)},datasets:[{{data:{json.dumps(values)},
    backgroundColor:{json.dumps(colors)},borderWidth:2,borderColor:'#ffffff'}}]}},
  options:{{responsive:true,maintainAspectRatio:false,
    ...
    with col_priority:
        ...
        if pr:
            ...
            colors = ["#1D9E75" if r>=80 else "#BA7517" if r>=50 else "#D85A30" for r in rates]
            html = f"""<!DOCTYPE html><html><head>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"></script>
</head><body style="margin:0;background:transparent">
<div style="position:relative;width:100%;height:220px"><canvas id="pr"></canvas></div>
<script>
new Chart(document.getElementById('pr'),{{
  type:'bar',
  data:{{labels:{json.dumps(labels)},datasets:[{{label:'Take rate %',data:{json.dumps(rates)},
    backgroundColor:{json.dumps(colors)},borderRadius:4,borderSkipped:false}}]}},
  options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},
    scales:{{y:{{min:0,max:100,ticks:{{callback:function(v){{return v+'%'}}}},
               grid:{{color:'rgba(0,0,0,0.06)'}}}},x:{{ticks:{{font:{{size:11}}}}}}}}}}
}});
</script></body></html>"""
            components.html(html, height=240)
    ...
    if stats["overall_counts"]:
        ...
        overall_labels = {"resolved":"Resolved","escalated":"Escalated",
                          "ongoing":"Ongoing","closed_unresolved":"Closed unresolved"}
        ov_colors = {"resolved":"#1D9E75","escalated":"#D85A30",
                     "ongoing":"#378ADD","closed_unresolved":"#A32D2D"}
        for status, count in ov.items():
            ...
            st.markdown(
                ...
                f'<span style="color:#888;font-size:12px;min-width:36px;text-align:right">{pct}%</span>'
...
def main():
    ...
    if total == 0:
        st.info("No incidents analyzed yet. Go to the main page and run an analysis.")
    ...
    c3.metric("Avg max exposure",
        f"${avg_fm/1_000_000:.1f}M" if avg_fm >= 1_000_000 else f"${avg_fm/1_000:.0f}K")
    crit_high = (stats["severity_counts"].get("CRITICAL",0) +
                 stats["severity_counts"].get("HIGH",0))
    c4.metric("Critical/High", crit_high)
    most_vuln = (max(stats["vuln_counts"], key=stats["vuln_counts"].get)
                 .replace("_"," ").title() if stats["vuln_counts"] else "—")
    ...
    with col_a:
        ...
        if stats["severity_counts"]: severity_pie(stats["severity_counts"])
        else: st.caption("No data yet.")
    with col_b:
        st.subheader("Vulnerability types")
        if stats["vuln_counts"]: vuln_bar(stats["vuln_counts"])
        else: st.caption("No data yet.")
    with col_c:
        st.subheader("Incident types")
        if stats["type_counts"]: type_bar(stats["type_counts"])
        else: st.caption("No data yet.")
    ...
    with col_d:
        ...
        if len(incidents) >= 2: risk_timeline(incidents)
        else: st.caption("Run at least 2 analyses to see the trend.")
    with col_e:
        st.subheader("Financial exposure per incident ($K)")
        if incidents: financial_bar(incidents)
        else: st.caption("No data yet.")

    st.divider()
    st.subheader("Decision audit trail")
    audit_stats = get_audit_stats(user_id=user_id)
    ...
    sev_filter = st.multiselect("Filter by severity",
        ["CRITICAL","HIGH","MEDIUM","LOW"], default=[], placeholder="All severities")
    type_filter = st.multiselect("Filter by type",
        list(stats["type_counts"].keys()), default=[], placeholder="All types",
        format_func=lambda x: x.replace("_"," ").title())

    all_rows = get_all_incidents(user_id=user_id)
    filtered = all_rows
    if sev_filter:  filtered = [r for r in filtered if r["severity_level"] in sev_filter]
    if type_filter: filtered = [r for r in filtered if r["incident_type"] in type_filter]

    if not filtered:
        st.info("No incidents match your filters.")
    else:
        st.caption(f"Showing {len(filtered)} incident(s)")
        color_map = {"CRITICAL":"#A32D2D","HIGH":"#D85A30","MEDIUM":"#BA7517","LOW":"#1D9E75"}
        ...
        for idx, row in enumerate(filtered):
            ...
            with st.expander(
                ...
                with left:
                    ...
                    st.markdown(f"**Vulnerability:** `{row['vulnerability_type'] or '—'}`")
                    st.markdown(f"**Recovery:** {row['recovery_timeline'] or '—'}")
                    if row.get("primary_root_cause"):
                        ...
                        st.markdown(
                            ...
                            f'{row["primary_root_cause"]}</div>', unsafe_allow_html=True)
                ...
                with right:
                    st.markdown(
                        ...
                        f'background:var(--color-background-secondary);border-radius:8px;margin-bottom:8px">'
                        f'<div style="font-size:28px;font-weight:600;color:{sev_color}">'
                        f'{row["risk_score_overall"] or "—"}/10</div>'
                        f'<div style="font-size:11px;color:#888">Risk score</div></div>',
                    ...
                    st.markdown(
                        ...
                        f'<div style="font-size:15px;font-weight:600">{fmt(fin_min)} – {fmt(fin_max)}</div>'
                        f'<div style="font-size:11px;color:#888">Financial exposure</div></div>',
                        unsafe_allow_html=True)
                    st.markdown("")
                    if st.button("Delete", key=f"del_{row['incident_id']}_{idx}", type="secondary"):
                        delete_incident(row["incident_id"], user_id=user_id)
